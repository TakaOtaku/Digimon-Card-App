...
import { compareIDs, levelSort } from '../../../functions/digimon-card.functions';
    ...
    <div [formGroup]="form" class="flex w-full flex-col">
      ...
      <div class="my-3 grid grid-cols-2 lg:grid-cols-4">
        ...
        <div class="flex flex-col">
          ...
          <p-dropdown styleClass="truncate w-full mr-1" [options]="sizeOptions" formControlName="size" appendTo="body" optionLabel="name">
          </p-dropdown>
        </div>
      </div>
...
export class DeckSubmissionComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnInit(): void {
    this.form
      ...
      .subscribe((deckList: string) => {
        ...
        this.deck ? this.mapDeck(this.deck, this.digimonCardStore.cards()) : null;
  ...
  private updateValues(inputDeck: IDeck) {
    ...
    inputDeck.cards.forEach((card) => {
      const dc = this.digimonCardStore.cards().find((dc) => compareIDs(dc.id, card.id));
