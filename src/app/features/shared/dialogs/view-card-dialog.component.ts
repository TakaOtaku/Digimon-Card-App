    ...
    <div class="h-full w-full min-w-full max-w-full overflow-x-hidden md:w-[700px] md:min-w-[700px] md:max-w-[700px]">
        ...
        <div class="md:max-w-1/2 w-full self-center md:w-1/2 md:pl-2">
          ...
          <div
            *ngIf="collectionMode"
            ...
            <p *ngIf="collectionCard" class="font-white ml-auto mr-1.5 font-bold leading-[1.7em]">{{ collectionCard.count }}x</p>
          </div>
          <div
            *ngIf="card.form !== '-'"
            class="my-0.5 flex w-full flex-row rounded-full border border-slate-200 backdrop-brightness-150"
          ...
          <div
            *ngIf="card.digivolveCondition.length > 0"
            class="my-0.5 flex w-full flex-row rounded-full border border-slate-200 backdrop-brightness-150">
            <p [ngStyle]="{ color }" class="text-black-outline-xs ml-1.5 text-lg font-extrabold">Digivolve</p>
            <div
              *ngFor="let digivolveCondition of card.digivolveCondition"
              ...
              <span class="absolute left-[5px] top-[2px] text-2xs">Lv. {{ digivolveCondition.level }}</span>
              <span class="absolute bottom-[0] left-[13px] text-sm font-bold">{{ digivolveCondition.cost }}</span>
            </div>
          </div>
          ...
          <div *ngIf="card.dnaDigivolve !== '-'" class="my-0.5 flex w-full flex-col rounded-full" id="Digimon-DNA-Digivolve">
            ...
            <span class="font-white whitespace-pre-wrap font-bold leading-[1.7em]" [innerHTML]="replaceWithImageTags(card.dnaDigivolve)">
            </span>
          </div>
      ...
      <div class="my-4 max-w-full" id="Effects">
        <div *ngIf="card.rule !== '-'" class="flex flex-col" id="Digimon-Rule">
          <p [ngStyle]="{ color }" class="text-black-outline-xs text-lg font-extrabold">Rule</p>
          <span class="font-white whitespace-pre-wrap font-bold" [innerHTML]="replaceWithImageTags(card.rule)"></span>
        </div>
        ...
        <div *ngIf="card.linkEffect !== '-'" class="flex flex-col" id="Digimon-Link">
          <p [ngStyle]="{ color }" class="text-black-outline-xs text-lg font-extrabold">Link</p>
          ...
          <div class="grid grid-cols-5">
            ...
            <span class="font-white whitespace-pre-wrap font-bold col-span-4" [innerHTML]="replaceWithImageTags(card.linkEffect)"></span>
          </div>
          <div class="grid grid-cols-5">
            <p class="font-bold">Link DP:</p>
            <span class="font-white whitespace-pre-wrap font-bold col-span-4" [innerHTML]="replaceWithImageTags(card.linkDP)"></span>
          </div>
        </div>
      </div>
...
export class ViewCardDialogComponent {
  ...
  previousCard() {
    const id = this.digimonCardStore.cards().findIndex((card) => this.card.id === card.id);
    if (id === -1 || id === 0) {
      return;
    }
