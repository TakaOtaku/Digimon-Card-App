...
@Component({
  ...
  template: `
    <div class="relative m-1 flex flex-col">
      <div class="group absolute z-[101] flex h-full w-full flex-col rounded xl:opacity-0 xl:hover:opacity-100" (click)="showCardDetails()" (contextmenu)="showCardDetails()">
        <div *ngIf="edit" class="hidden h-1/2 w-full rounded bg-sky-700 bg-opacity-30 xl:block">
          <div class="absolute top-2">
            <button (click)="addCardCount($event)" type="button" class="z-[101] mx-auto flex hidden h-8 w-8 translate-x-full rounded border border-black bg-sky-700 group-hover:block">
        ...
        <div *ngIf="edit" class="hidden h-1/2 w-full rounded bg-red-500 bg-opacity-30 xl:block">
          <div class="absolute bottom-2">
            <button (click)="reduceCardCount($event)" type="button" class="z-[101] mx-auto flex hidden h-8 w-8 translate-x-full rounded border border-black bg-sky-700 group-hover:block">
              <span class="text-shadow text-2xl font-black text-[#e2e4e6]"> - </span>
            </button>
          </div>
        </div>
      ...
      <span class="text-shadow-white absolute bottom-8 right-1 z-[100] text-2xl font-black text-orange-500 sm:text-3xl lg:text-2xl xl:bottom-1">
        <span class="text-sky-700">x</span>{{ card.count }}
      </span>

      <p *ngIf="missingCards" class="text-red text-black-outline absolute left-0 right-0 top-[40px] z-[100] mx-auto text-center text-6xl font-bold">
        {{ (cardHave ?? 0) - card.count > 0 ? 0 : (cardHave ?? 0) - card.count }}
      </p>
    </div>

    <p-dialog [(visible)]="viewCardDialog" [showHeader]="false" [modal]="true" [dismissableMask]="true" [resizable]="false" styleClass="overflow-x-hidden" (close)="viewCardDialog = false">
