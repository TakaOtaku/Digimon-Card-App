...
export const digimonReducer = createReducer(
  ...
  on(addToDeck, (state, { card }) => {
    if (state.deck) {
      const foundCard = state.deck.cards.find(
        (deckCard) => deckCard.id === card.id
      );
      if (foundCard) {
        //Increase Card Count
        const cards = state.deck.cards.filter(
          (deckCard) => deckCard.id !== card.id
        );
  ...
  on(setExportDeckDialog, (state, { show, deck }) => ({
    ...state,
    dialogs: { ...state.dialogs, exportDeck: { show, deck } },
  })),
  on(setImportDeckDialog, (state, { show }) => ({
    ...state,
    dialogs: { ...state.dialogs, importDeck: { show } },
  })),
  on(setAccessoryDeckDialog, (state, { show, deck }) => ({
    ...state,
    dialogs: { ...state.dialogs, accessoryDeck: { show, deck } },
  })),
  on(setViewCardDialog, (state, { show, card }) => ({
    ...state,
    dialogs: { ...state.dialogs, viewCard: { show, card } },
  }))
);
