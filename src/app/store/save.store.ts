...
export const SaveStore = signalStore(
  ...
  withMethods((store, toastrService = inject(ToastrService), authService = inject(AuthService)) => ({
        ...
        switchMap(() => {
          return authService.loadSave().pipe(
            tapResponse({
              ...
              error: () => {
                toastrService.info('There was an error while loading your save, please refresh the site!', 'Save Loading Error!');
              },
    ...
    importDeck(deck: IDeck): void {
      patchState(store, (state) => {
        ...
        const foundDeck = state.save.decks?.find((value) => value.id === deck.id);
        if (foundDeck) {
          const allButFoundDeck: IDeck[] = state.save.decks.filter((value) => value.id !== deck.id);
    ...
    deleteDeck(deck: IDeck): void {
      patchState(store, (state) => {
        ...
        const decks = [...new Set(state.save.decks.filter((item) => item.id !== deck.id))];
        return { save: { ...state.save, decks } };
      });
    },
  })),
