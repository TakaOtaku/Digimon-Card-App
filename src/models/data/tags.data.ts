import { ITag } from '../interfaces';

export const TAGS = [
  'AD1',
  'BT1',
  'BT2',
  'BT3',
  'BT4',
  'BT5',
  'BT6',
  'BT7',
  'BT8',
  'BT9',
  'BT10',
  'BT11',
  'BT12',
  'BT13',
  'BT14',
  'BT15',
  'BT16',
  'BT17',
  'BT18',
  'BT19',
  'BT20',
  'BT21',
  'BT22',
  'BT23',
  'BT24',
  'BT25',
  'BT26',
  'RB1',
  'LM',
  'EX1',
  'EX2',
  'EX3',
  'EX4',
  'EX5',
  'EX6',
  'EX7',
  'EX8',
  'EX9',
  'EX10',
  'EX11',
  'EX12',
  'ST1',
  'ST2',
  'ST3',
  'ST4',
  'ST5',
  'ST6',
  'ST7',
  'ST8',
  'ST9',
  'ST10',
  'ST12',
  'ST13',
  'ST14',
  'ST15',
  'ST16',
  'ST17',
  'ST18',
  'ST19',
  'ST20',
  'ST21',
  'ST22',
  'ST23',
  'ST24',
];

export const tagsList: ITag[] = [
  { name: 'BT1', color: 'Primary' },
  { name: 'BT2', color: 'Primary' },
  { name: 'BT3', color: 'Primary' },
  { name: 'BT4', color: 'Primary' },
  { name: 'BT5', color: 'Primary' },
  { name: 'BT6', color: 'Primary' },
  { name: 'BT7', color: 'Primary' },
  { name: 'BT8', color: 'Primary' },
  { name: 'BT9', color: 'Primary' },
  { name: 'BT10', color: 'Primary' },
  { name: 'BT11', color: 'Primary' },
  { name: 'BT12', color: 'Primary' },
  { name: 'BT13', color: 'Primary' },
  { name: 'BT14', color: 'Primary' },
  { name: 'BT15', color: 'Primary' },
  { name: 'BT16', color: 'Primary' },
  { name: 'BT17', color: 'Primary' },
  { name: 'BT18', color: 'Primary' },
  { name: 'BT19', color: 'Primary' },
  { name: 'BT20', color: 'Primary' },
  { name: 'BT21', color: 'Primary' },
  { name: 'BT22', color: 'Primary' },
  { name: 'BT23', color: 'Primary' },
  { name: 'BT24', color: 'Primary' },
  { name: 'BT25', color: 'Primary' },
  { name: 'BT26', color: 'Primary' },

  { name: 'EX1', color: 'Primary' },
  { name: 'EX2', color: 'Primary' },
  { name: 'EX3', color: 'Primary' },
  { name: 'EX4', color: 'Primary' },
  { name: 'EX5', color: 'Primary' },
  { name: 'EX6', color: 'Primary' },
  { name: 'EX7', color: 'Primary' },
  { name: 'EX8', color: 'Primary' },
  { name: 'EX9', color: 'Primary' },
  { name: 'EX10', color: 'Primary' },
  { name: 'EX11', color: 'Primary' },
  { name: 'EX12', color: 'Primary' },

  { name: 'AD1', color: 'Primary' },
  { name: 'RB1', color: 'Primary' },

  { name: 'LM', color: 'Primary' },

  { name: 'ST1', color: 'Primary' },
  { name: 'ST2', color: 'Primary' },
  { name: 'ST3', color: 'Primary' },
  { name: 'ST4', color: 'Primary' },
  { name: 'ST5', color: 'Primary' },
  { name: 'ST6', color: 'Primary' },
  { name: 'ST7', color: 'Primary' },
  { name: 'ST8', color: 'Primary' },
  { name: 'ST9', color: 'Primary' },
  { name: 'ST10', color: 'Primary' },
  { name: 'ST11', color: 'Primary' },
  { name: 'ST12', color: 'Primary' },
  { name: 'ST13', color: 'Primary' },
  { name: 'ST14', color: 'Primary' },
  { name: 'ST15', color: 'Primary' },
  { name: 'ST16', color: 'Primary' },
  { name: 'ST17', color: 'Primary' },
  { name: 'ST18', color: 'Primary' },
  { name: 'ST19', color: 'Primary' },
  { name: 'ST20', color: 'Primary' },
  { name: 'ST21', color: 'Primary' },
  { name: 'ST22', color: 'Primary' },
  { name: 'ST23', color: 'Primary' },
  { name: 'ST23', color: 'Primary' },

  { name: 'Illegal', color: 'Primary' },
];
