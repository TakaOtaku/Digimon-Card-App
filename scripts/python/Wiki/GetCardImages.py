...
def upload_to_bunny(local_path, image_id):
  ...
  try:
    ...
    success = obj_storage.PutFile(f"{os.path.basename(local_path)}", None, './scripts/python/Wiki/digimon-images/')
  ...
  for item in gallery_items:
    ...
    if img is None:
      ...
      if id_with_p in existing_cdn_files or f"{id_with_p}.webp" in existing_cdn_files:
        ...
        local_path_webp = os.path.join(temp_dir, f"{id_with_p}.webp")

        if await download_image(session, save_location, local_path):
          print(f"Downloaded image {id_with_p} successfully. Saved to {local_path}")

          TransformPNG.singlePngToWebP(local_path, id_with_p)

          # Upload to BunnyCDN
          if upload_to_bunny(local_path_webp, id_with_p):
            # Remove local file after successful upload
            try:
              print('Removing Files from OS')
              os.remove(local_path)
              os.remove(local_path_webp)
            except:
              print('Error while removing Files from OS')
              pass
